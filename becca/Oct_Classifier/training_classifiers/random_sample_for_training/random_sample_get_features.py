...
def bag_of_words(title, stop_words):
    ...
    if not pd.isna(row['VIAF_titlelist']) and not pd.isna(row['S2_titlelist']):
        ...
        if isinstance(s_list, list):
            ...
            for title in s_list:
                title = str(title)
        if isinstance(v_list, list):
            v_bag = bag_of_words(v_list, stop_words)
        if isinstance(s_list, list):
            s_bag = bag_of_words(s_list, stop_words)
...
def find_word_overlap(row):
    if not pd.isna(row['VIAF_titlelist']) and not pd.isna(row['S2_titlelist']):
        ...
        if isinstance(v_list, list):
            for title in v_list:
                title = str(title)
                v_bag.update(bag_of_words(title, stop_words))  # Update the set with words

        # Create bag of words for S2 titles
        if isinstance(s_list, list):
            for title in s_list:
                title = str(title)
                s_bag.update(bag_of_words(title, stop_words))  # Update the set with words
...
def main(S2_data_dict, df):
    ...
    for idx, row in df.iterrows():
        if row['word_overlap_count'] > 0:
            ...
            cosine_matches, sim_scores = cosine_similarity_titles(S2_titlelist, VIAF_titlelist, threshold=0.7)
            ...
            if cosine_matches:
                ...
                df.at[idx, 'sim_scores'] = sim_scores
            else:
                df.at[idx, 'cosine_matches'] = 0
                df.at[idx, 'sim_scores'] = 0
