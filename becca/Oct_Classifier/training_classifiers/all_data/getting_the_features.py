...
def find_avg_pubdate(pubdates):
        ...
        if isinstance(pubdates, float):
            ...
            if len(pubdates) > 5:
                ...
                avg_pubdate = pubdates
                return avg_pubdate
        if isinstance(pubdates, str) and 'no date' not in pubdates and pubdates != "" and  pubdates != 'nan':
...
def main(df_4, S2_data_dict):
    ...
    for idx, row in df.iterrows():
        ...
        if pubdates is not None or pubdates != 'nan':
            avg_pubdate = find_avg_pubdate(pubdates)
