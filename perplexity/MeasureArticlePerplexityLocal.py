...
def all_word_masking(features):
	...
	for i, feature in enumerate(features):
		...
		wordpairs = create_paired_list(mapping_keys)
...
model_dir1 = 'novelty/perplexity/from' + str(modelfloor) + 'to' + str(modelceiling)
...
# Check if Metal is supported and use it
...
for year in range(floor, ceiling + 1):
	...
	for paper in paperIds_in_year:
		...
		tokenized_dataset = tokenized_dataset.map(remove_keys, batched=False)
		...
		for key, masked_examples in tqdm(masked_dataset_dict.items(), desc = 'texts:' ):
